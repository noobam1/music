...
OWNER_ID = "708030615"  # Replace with your actual owner ID (as a string)
...
# Admin IDs
admin_id = {"708030615"}  # Existing admin IDs (as strings)
...
async def list_users(update: Update, context: CallbackContext):
    ...
    if user_id not in admin_id and user_id != OWNER_ID:
...
async def start_command(update: Update, context: CallbackContext):
    ...
    if str(update.message.chat.id) in admin_id or str(update.message.chat.id) == OWNER_ID:
...
async def handle_attack(update: Update, context: CallbackContext):
    ...
    if user_id in users and users[user_id]:
        ...
        await update.message.reply_text("⛔️ 𝗨𝗻𝗮𝘂𝘁𝗵𝗼𝗿𝗶𝘀𝗲𝗱 𝗔𝗰𝗰𝗲𝘀𝘀! ⛔️\n\n OWNER :- @NOOB_AM !")
    ...
    if len(details) == 3:
        ...
        try:
            ...
            if attack_time > MAX_ATTACK_TIME:
                ...
                full_command = f"./noobam {target} {port} {attack_time} 1200 1200"
...
async def my_info(update: Update, context: CallbackContext):
    ...
    if user_id in admin_id or user_id == OWNER_ID:
        role = "Admin"
...
# /mycoins command
async def my_coins(update: Update, context: CallbackContext):
    if update.message is None:
        return
    user_id = str(update.message.from_user.id)
    if user_id not in coins:
        await update.message.reply_text("⚠️ You do not have any coins.")
        return
    await update.message.reply_text(f"💰 Your coin balance: {coins[user_id]}")
...
async def add_coins(update: Update, context: CallbackContext):
    ...
    user_id = str(update.message.from_user.id)
    if user_id != OWNER_ID:
        await update.message.reply_text("⛔ Access Denied: Only the owner can add coins.")
        return
    ...
    if target_user_id not in coins:
        coins[target_user_id] = 0
    coins[target_user_id] += amount
    save_coins()
    await update.message.reply_text(f"✅ Added {amount} coins to user {target_user_id}. New balance: {coins[target_user_id]}")
...
async def main():
    ...
    application.add_handler(CommandHandler("mycoins", my_coins))
